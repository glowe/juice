(function(juice, proj, jQuery) {
     ...
     lib.define = function(spec) {
         ...
         var rpc = function(args, success_fn, failure_fn) {
             ...
             return find_proxy(rpc).execute(
                 {rpc: rpc,
                  args: args,
                  success_fn: function(rsp) {
                      call_when_finished();
                      try {
                          do_validate('rsp_spec', rsp.data);
                          success_fn(rsp);
                      }
                      catch (e) {
     ...
     how_to_mock_namespace = function(ns) {
         var mock;
         if (!is_mocking_enabled) {
             return null;
         }
     ...
     lib.mock_namespace = function(namespace, failure) {
         var mock;
     ...
     find_proxy = function(rpc) {
         var how_to_mock = how_to_mock_namespace(rpc.namespace);
         if (!juice.is_null(how_to_mock)) {
             return how_to_mock ? failure_mocking_proxy : success_mocking_proxy;
         }
     ...
     var make_proxy_startable = function(fn) {
         var
         that = {},
         pending_requests = [],
         started = false;
     ...
     success_mocking_proxy = make_proxy_startable(
         function(requests) {
             juice.foreach(
                 requests,
                 function(req) {
                     call_with_random_delay(
                         req.rpc,
                         function() {
                             req.success_fn({outcome: 'success',
                                                 data: (juice.is_function(req.rpc.mock)
                                                        ? req.rpc.mock(req.args) : req.rpc.mock)});
                         });
                 });
         });

     failure_mocking_proxy = make_proxy_startable(
         function(requests) {
            juice.foreach(
                requests,
                function(req) {
                    var rsp = {outcome: 'failure', message: 'mocked failure'};
                    call_with_random_delay(
                        req.rpc,
                        function() {
                            if (req.failure_fn) {
     ...
     lib.start = function() {
         ...
         default_proxy.start();
         if (is_mocking_enabled()) {
             success_mocking_proxy.start();
             failure_mocking_proxy.start();
         }
     };
...
(function(juice, proj, jQuery) {
     ...
     /* Returns an object that encapsulates some of the details
      oassociated with creating a boxcar of RPC requests. */
     ...
     juice.rpc.create_basic_boxcar_proxy = function(rpc_url) {
         ...
         return function(requests) {
             var boxcar_helper = make_boxcar_helper(requests,
                                                    function(req, rsp) {
                                                        if (rsp.outcome === 'success') {
                                                            req.success_fn(rsp);
                                                        }
                                                        else if (req.failure_fn) {
                                                            req.failure_fn(rsp);
                                                        }
                                                        else {
                                                            juice.event.publish('service-failure', rsp);
