...
* STARTED Clean up paged_list code in bar. Checkable, filterable enhancers and then also change everything
to use proj.widget.bar.link.
* STARTED Ensure that the interface of the standard form widgets are consistent
...
* STARTED Remove dead widgets
<<<<<<< HEAD:docs/TODO.org
* STARTED Add hrefs to all links in standard library components + demo
...
* IMPORTANT Compiled script names should have something unique in them to thwart browser caching
