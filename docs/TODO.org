...
* MAYBE Why do we have form.init?
...
* IMPORTANT Change widget package scoping
  We shouldn't bother scoping files anymore
* IMPORTANT RPC packages should probably be in single files
* IMPORTANT Revisit JUICE_LIBPATH--it doesn't really work the way we expected.
* IMPORTANT Upgrade jquery
* IMPORTANT Add juice.page.define_external
* IMPORTANT Provide convenient way to globally manipulate page titles.
* IMPORTANT Need to recompile base whenever an rpc/widget package is added or removed.
...
* MAYBE support optional page parameters and default values
