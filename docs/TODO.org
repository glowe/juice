...
* IMPORTANT RPC back-end doesn't handle certain catastrophic failure scenarios, e.g. a failed require_once in PHP.
* IMPORTANT Server-side RPC module should test that the registered RPC function implementation exists, and provide a good error message
...
* IMPORTANT Provide a better error message when putting widgets into a div with nested divs
  For example,
...
* IMPORTANT Analytics (D & G)
* NEXT View note page needs styling (Eloy)
* NEXT Fix page titles (D & G)
* NEXT Fix page CSS URLs (D & G)
...
* IMPORTANT Provide convenient way to globally manipulate page titles.
...
* MAYBE More modifiers
  ...
  - [ ] ellipsis formatter
* MAYBE Change the way we scope individual js files in packages.
