...
* STARTED automatically generate form html/template based on abstract description of form inputs
  [ ] Double-click protection
  [ ] General form error label at top of form?
  [ ] RPC to load data? lazy-loaded form input?
  [ ] Is form input ready? and disable form until it is
  [ ] Preloading a form input with values. Is this different than with ref data from an RPC?
...
* IMPORTANT revise the way decorators are defined and organized
...
>>>>>>> f94497f39f8be1558ccbbd6aec08b61470efe288:docs/TODO.org
=======
...
* MAYBE HTML tag generator
  so that we don't run into issues with quotes
* MAYBE do another pass on error reporting; make sure we're using juice.error.raise correctly everywhere
